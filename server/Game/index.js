class gameHandler {
	...
	collide = (instance, other) => {
		...
		if (
			instance.settings.no_collisions ||
		...
		switch (true) {
			case instance.isPortal || other.isPortal:
				let [portal, otherBody] = instance.isPortal
					? [instance, other]
					: [other, instance];
				...
				if (
					...
					global.gameManager.socketManager.sendToServer(
						otherBody.socket,
						portal.settings.destination
					);
				} else if (
			...
			case instance.settings.hitsOwnType === other.settings.hitsOwnType:
				switch (instance.settings.hitsOwnType) {
					case 'assembler': {
						...
						const better = state =>
							target1[state] > target2[state] ? target1[state] : target2[state];
		...
		for (const instance of entities.values()) {
			if (instance.contemplationOfMortality() === 1) {
				if (
					Config.OUTBREAK &&
					!instance.zombified &&
					(instance.isPlayer || instance.isBot)
				) {
			...
			for (const other of grid
				.query(instance.minX, instance.minY, instance.maxX, instance.maxY)
				.values()) {
	...
	foodloop() {
		...
		let totalFoods = 1;
		if (Config.food_group_cap) {
			if (Math.random() < 0.2) {
				// 1/5 chance to spawn a group
		...
		if (
			Math.random() < 1 / 3 &&
			global.gameManager.room.spawnable[TEAM_ENEMIES]
		) {
			// Enemy spawn
			while (
				Math.random() < 1 / 3 &&
				this.enemyFoods.length < Config.enemy_cap_nest
			) {
				const tile = ran
					.choose(global.gameManager.room.spawnable[TEAM_ENEMIES])
					.randomInside();
	...
	maintainloop = () => {
		...
		for (let i = 0; i < this.bots.length; i++) {
			let o = this.bots[i];
			if (
				o.skill.level < Config.level_cap &&
				o.skill.level >= Config.bot_start_level
			) {
		...
		if (
			this.checkUsers() &&
			Config.bosses_spawn &&
			!this.naturallySpawnedBosses.length &&
			this.bossTimer++ > Config.boss_spawn_cooldown
		) {
			...
			setSyncedTimeout(() => {
				...
				for (let i = 0; i < amount; i++) {
					...
					boss.on('dead', () =>
						util.remove(
							this.naturallySpawnedBosses,
							this.naturallySpawnedBosses.indexOf(boss)
						)
					);
				}
	...
	quickMaintainLoop = () => {
		...
		if (
			!global.gameManager.arenaClosed &&
			!global.cannotRespawn &&
			this.bots.length < Config.bot_cap
		) {
			let team =
					Config.mode === 'tdm' || Config.mode === 'tag'
						? getWeakestTeam(global.gameManager)
						: undefined,
	...
	spawnBots(loc, team) {
		...
		let color = Config.random_body_colors
		...
		setTimeout(() => {
			...
			o.define(
				{
					CONTROLLERS: CC.CONTROLLERS
						? [...Class.bot.CONTROLLERS, ...CC.CONTROLLERS]
						: Class.bot.CONTROLLERS,
				...
				o.define(
					{
						CONTROLLERS: [
							...
							[
								'wanderAroundMap',
								{ immitatePlayerMovement: true, lookAtGoal: true }
							]
						],
						FACING_TYPE: CC.FACING_TYPE
							? CC.FACING_TYPE
							: Class.bot.FACING_TYPE,
						AI: Class.bot.AI
					},
					false,
					true,
			...
			o.on('define', () => {
				...
				o.define(
					...
					true,
					false
				); // Just reoverride the facing type.
			});
