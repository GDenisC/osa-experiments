...
class Entity extends EventEmitter {
	constructor(position, master) {
		...
		this.confinement = {
			xMin: 0,
			xMax: global.gameManager.room.width,
			yMin: 0,
			yMax: global.gameManager.room.height
		};
		...
		// Initialize physics and collision
	...
	define(defs, emitEvent = true, overwriteindex = true, overrideDefs = true) {
		...
		if (set.SHAPE != null) {
			this.shape =
				typeof set.SHAPE === 'number' ? set.SHAPE : set.SHAPE_NUM ?? 0;
		...
		if (set.CAN_SEE_INVISIBLE_ENTITIES != null)
			this.settings.canSeeInvisible = set.CAN_SEE_INVISIBLE_ENTITIES;
		if (set.NECRO != null) {
			...
			for (let shape of this.settings.necroTypes) {
				...
				this.settings.necroDefineGuns[shape] = Array.from(this.guns).filter(
					gun =>
						gun[1].bulletType.NECRO &&
						(gun[1].bulletType.NECRO === shape ||
							(gun[1].bulletType.NECRO === true &&
								gun[1].bulletType.SHAPE === this.shape) ||
							gun[1].bulletType.NECRO.includes(shape))
				)[0];
			}
		...
		if (set.RENDER_ON_LEADERBOARD != null)
			this.settings.renderOnLeaderboard = set.RENDER_ON_LEADERBOARD;
		...
		if (set.TEAM != null) {
			...
			if (!global.gameManager.socketManager.players.length) {
				...
				for (
					let i = 0;
					i < global.gameManager.socketManager.players.length;
					i++
				) {
					if (
						global.gameManager.socketManager.players[i].body.id == _entity.id
					) {
		...
		if (set.ARENA_CLOSER != null)
			(this.isArenaCloser = set.ARENA_CLOSER), (this.ac = set.ARENA_CLOSER);
		...
		if (set.NO_SIZE_ANIMATION != null)
			this.settings.noSizeAnimation = set.NO_SIZE_ANIMATION;
		if (set.LEVEL != null) {
			this.skill.reset();
		...
		if (set.SKILL_CAP != null) {
			let skillCapsToSet = Array.isArray(set.SKILL_CAP)
				? set.SKILL_CAP
				: SKILL_ORDER.map(
						name =>
		...
		if (set.SKILL != null) {
			let skillsToSet = Array.isArray(set.SKILL)
				? set.SKILL
				: SKILL_ORDER.map(
						name => (set.SKILL[name] !== undefined ? set.SKILL[name] : 0) // Default current skill points to 0, cant decide if it should be 9 or 0
				  );
		...
		if (set.CONNECT_CHILDREN_ON_CAMERA)
			this.settings.connectChildrenOnCamera = true;
		...
		if ('function' === typeof set.defineLevelSkillPoints)
			this.skill.LSPF = set.defineLevelSkillPoints;
			...
			set.SHAKE.forEach(set => {
				if (set.CAMERA_SHAKE) {
					info.push({
						...
						push: set.PUSH ?? false,
						applyOn: {
							// You can basically add other stuff to trigger the shake.
		...
		// Make the entity targetable if they arent a bullet, etc.
		const checkIfTargetAble = [
			'bullet',
			'drone',
			'swarm',
			'trap',
	...
	get size() {
		...
		if (this.settings.healthWithLevel) {
			...
			if (Config.defineGrowthMultiplier && (this.underControl || this.isPlayer || this.isBot)) {
				levelMultiplier = Config.defineGrowthMultiplier(levelMultiplier, this.skill.score, level, this);
			}
		}
	...
	camera() {
		// Get bound data
		const turretsAndProps = Array.from(this.turrets).concat(
		...
		const typeValue =
			(this.settings.drawHealth ? 0x02 : 0) +
			((this.type === 'tank' || this.type === 'miniboss') && this.displayName
				? 0x04
				: 0);
		...
		const cameraInfo = {
			...
			twiggle:
				forceTwiggle.includes(this.facingType) ||
				this.eastereggs.braindamage ||
				this.settings.connectChildrenOnCamera ||
	...
	upgrade(number, branchId) {
		...
		if (number.isDailyUpgrade) {
			...
			if (
				requestedIndex === ensureIsClass(Config.daily_tank.tank).index &&
				this.skill.level >= Config.tier_multiplier * Config.daily_tank.tier
			) {
				if (hasWatchedAd) {
					upgraded = true;
					this.upgrades = [];
	...
	physics() {
		...
		this.stepRemaining = 1;
		this.x +=
			(this.stepRemaining * this.velocity.x) / global.gameManager.roomSpeed;
		this.y +=
			(this.stepRemaining * this.velocity.y) / global.gameManager.roomSpeed;
	}
	...
	confinementToTheseEarthlyShackles() {
		...
		if (!this.settings.canGoOutsideRoom) {
			if (Config.arena_shape === 'circle') {
				...
				if (
					dist >
					global.gameManager.room.width - global.gameManager.room.width / 2
				) {
					let strength =
						((dist - global.gameManager.room.width / 2) *
				...
				this.accel.x -=
					(Math.min(
						this.x - this.realSize + global.gameManager.room.width / 2 + 50,
						0
					) *
						Config.room_bound_force) /
				...
				this.accel.y -=
					(Math.min(
						this.y - this.realSize + global.gameManager.room.height / 2 + 50,
						0
					) *
						Config.room_bound_force) /
					global.gameManager.roomSpeed;
	...
	contemplationOfMortality() {
		...
		if (
			...
			this.health.amount -= this.health.getDamage(
				1 / global.gameManager.roomSpeed
			);
