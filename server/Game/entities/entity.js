...
class Entity extends EventEmitter {
			...
			set.SHAKE.forEach(set => {
				if (set.CAMERA_SHAKE) {
					info.push({
						...
						push: set.PUSH ?? false,
						applyOn: {
							// You can basicly add other stuff to trigger the shake.
	...
	refreshBodyAttributes() {
		...
		const growthLevel = Config.GROWTH
			...
			: 0;
		const speedReduce = Math.min(4, this.size / (this.coreSize || this.SIZE));
		this.acceleration =
			...
			case 'swarm':
				...
				if (gactive && l > this.size) {
					let desiredxspeed = (this.topSpeed * g.x) / l,
						...
						turning = Math.sqrt(
							(this.topSpeed *
								Math.max(1, this.motionTypeArgs.turnVelocity ?? this.range) +
								1) /
								a
						);
					engine = {
	...
	face() {
		...
		let defaultBound = () => {
			...
			if (this.control.main) {
				if (this.master.master.isPlayer) {
					let reverse = this.master.master.reverseTargetWithTank
						? 1
						: this.master.master.reverseTank;
			...
			this.facing += util.loopSmooth(
				this.facing,
				givenangle,
				4 / global.gameManager.runSpeed
			);
		};
		switch (this.facingType) {
			case 'spin':
				this.facing +=
					(this.facingTypeArgs.speed ?? 0.05) / global.gameManager.runSpeed;
				break;
			case 'spinWhenIdle':
				if (t && this.control.fire) this.facing = Math.atan2(t.y, t.x);
				else
					this.facing +=
						(this.facingTypeArgs.speed ?? 0.05) / global.gameManager.runSpeed;
				break;
			case 'turnWithSpeed':
				this.facing +=
					((this.velocity.length / 90) * Math.PI) /
			...
			case 'looseWithMotion':
				this.facing += util.loopSmooth(
					...
					4 / global.gameManager.roomSpeed
				);
				break;
			case 'withTarget':
			case 'toTarget':
			...
			case 'smoothToTarget':
				...
				break;
			case 'noFacing':
				if (this.lastSavedFacing !== this.facing)
		...
		this.vfacing =
			util.angleDifference(oldFacing, this.facing) *
			global.gameManager.roomSpeed;
	}
