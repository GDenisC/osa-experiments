...
class io_moveInCircles extends IO {
	...
	think() {
		if (!this.timer--) {
			...
			this.pathAngle -=
				(((this.body.velocity.length / 90) * Math.PI) /
					global.gameManager.runSpeed) *
				5;
		}
		return {
			goal: this.goal,
...
class io_listenToPlayer extends IO {
	...
	think() {
		...
		if (
			...
			!this.player.body.settings.braindamagemode
		) {
			let kk =
...
class io_nearestDifferentMaster extends IO {
	...
	validate(e, m, mm, sqrRange, sqrRangeMaster) {
		...
		if (theirMaster.team === myMaster.team || theirMaster.team === TEAM_ROOM)
			return false;
		...
		if (!(aiSettings.seeInvisible || this.body.isArenaCloser || e.alpha > 0.5))
			return false;
		if (!io_nearestDifferentMaster.validEntityTypes.has(e.type)) {
			if (
				(aiSettings.IGNORE_SHAPES || myMaster.aiSettings.IGNORE_SHAPES) &&
				e.type === 'food'
			)
				return false;
		}
	...
	buildList(range) {
		...
		const finalTargets = validCandidates
			.filter(e => {
				// Even more expensive
...
class io_wanderAroundMap extends IO {
	constructor(body, opts = {}) {
		...
		this.spot = ran
			.choose(global.gameManager.room.spawnableDefault)
			.randomInside();
	...
	think(input) {
		if (Config.BOT_MOVE && this.botMoveEnabled) {
			...
			for (let e of Config.BOT_MOVE) {
				if (
					(e.TEAM === 'any' || this.body.team == e.TEAM) &&
					e.MOVEMENT &&
					!input.fire
				) {
					if (!this.moveArray)
						(this.botMove_active = true),
							(this.moveArray = 0),
							(this.arrayLength = e.MOVEMENT.length - 1); // Set flags
					...
					if (
						new Vector(
							this.body.x - locX * 30,
							this.body.y - locY * 30
						).isShorterThan(e.RANGE ?? 50)
					) {
						if (this.moveArray == this.arrayLength)
							(this.botMoveEnabled = false), (this.enabled = true);
						this.moveArray++;
					}
					if (input.goal == null && !this.body.autoOverride) {
						let loc = compressMovement(this.body, {
							x: locX * 30,
							y: locY * 30
						});
						return {
		...
		if (this.enabled) {
			if (this.bossWander) {
				let points = [
					...
					},
					{
						x:
							...
							global.gameManager.room.width / this.howFarAwayFromEdgeOfMap, // top right
						y: global.gameManager.room.height / this.howFarAwayFromEdgeOfMap
					},
					{
						x:
							global.gameManager.room.width -
							global.gameManager.room.width / this.howFarAwayFromEdgeOfMap, // bottom right
						y:
							global.gameManager.room.height -
							global.gameManager.room.height / this.howFarAwayFromEdgeOfMap
					},
					{
				...
				if (
					this.tick >=
					100 + distanceFromPoint + (this.body.SPEED < 5 ? 1000 : 0)
				) {
					this.tick = 0;
			...
			if (
				...
				wouldHitWall(this.body, this.spot, true)
			) {
				this.spot = ran
					.choose(global.gameManager.room.spawnableDefault)
					.randomInside();
...
class io_formulaTarget extends IO {
	...
	think() {
		...
		let angle =
			this.originAngle +
			this.formula((this.frame += 1 / global.gameManager.runSpeed), this.body);
		return {
			goal: {
...
class io_orbit extends IO {
	...
	think(input) {
		let invertFactor = this.invert ? -1 : 1,
			master = this.body.master.useOwnMaster
				? this.body.master
				: this.body.master.master,
...
class io_snake extends IO {
	constructor(body, opts = {}) {
		...
		this.body.x +=
			Math.cos(this.body.velocity.direction) *
				this.body.size *
				Config.bullet_spawn_offset +
			0;
		this.body.y +=
			Math.sin(this.body.velocity.direction) *
				this.body.size *
				Config.bullet_spawn_offset +
			0;
		// Clamp scale to [45, 75]
		// Attempts to get the bullets to intersect with the cursor
		this.waveHorizontalScale = util.clamp(
			util.getDistance(this.body.master.master.control.target, { x: 0, y: 0 }) /
				Math.PI,
			45,
			75
		);
	...
	think(input) {
		...
		let waveX =
			(this.waveHorizontalScale * (this.body.RANGE - this.body.range)) /
			this.wavePeriod;
		let waveY =
			this.waveAmplitude *
				Math.sin(waveX / this.waveHorizontalScale) *
				this.waveInvert *
				this.reverseWave +
			this.yOffset;
		// Rotate the sin wave
		...
		this.velocityMagnitude = Math.min(
			0.1,
			this.velocityMagnitude + 0.01 / global.gameManager.runSpeed
		);
	}
}
