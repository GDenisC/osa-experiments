...
global.getWeakestTeam = () => {
	...
	teamCounts = Object.entries(teamCounts).map(([teamId, amount]) => {
		let weight =
			teamId in Config.team_weights ? Config.team_weights[teamId] : 1;
			...
			case 'swarm':
				...
				if (gactive && l > my.size) {
					let desiredxspeed = (my.topSpeed * g.x) / l,
						...
						turning = Math.sqrt(
							(my.topSpeed *
								Math.max(1, my.motionTypeArgs.turnVelocity ?? my.range) +
								1) /
								a
						);
					engine = {
...
global.runFace = (() => {
	return my => {
		...
		let defaultBound = () => {
			...
			if (my.control.main) {
				if (my.master.master.isPlayer) {
					let reverse = my.master.master.reverseTargetWithTank
						? 1
						: my.master.master.reverseTank;
		...
		switch (my.facingType) {
			case 'spin':
				my.facing +=
					(my.facingTypeArgs.speed ?? 0.05) / global.gameManager.runSpeed;
				break;
			case 'spinWhenIdle':
				if (t && my.control.fire) my.facing = Math.atan2(t.y, t.x);
				else
					my.facing +=
						(my.facingTypeArgs.speed ?? 0.05) / global.gameManager.runSpeed;
				break;
			case 'turnWithSpeed':
				my.facing +=
					(((my.velocity.length / 90) * Math.PI) /
						global.gameManager.roomSpeed) *
					(my.facingTypeArgs.multiplier ?? 1);
				break;
			case 'withMotion':
			...
			case 'smoothToTarget':
				...
				break;
			case 'noFacing':
				if (my.lastSavedFacing !== my.facing)
					my.facing = my.facingTypeArgs.angle ?? 0;
		...
		my.vfacing =
			util.angleDifference(oldFacing, my.facing) * global.gameManager.roomSpeed;
	};
})();
global.defineSplit = (() => {
	return (defs, branch, set, my, emitEvent) => {
		...
		if (set.BODY != null) {
			if (set.BODY.ACCELERATION != null)
				my.ACCELERATION *= set.BODY.ACCELERATION;
			...
			if (set.BODY.SHOCK_ABSORB != null)
				my.SHOCK_ABSORB *= set.BODY.SHOCK_ABSORB;
			if (set.BODY.RECOIL_MULTIPLIER != null)
				my.RECOIL_MULTIPLIER *= set.BODY.RECOIL_MULTIPLIER;
...
global.checkIfInView = (() => {
	return (boolean, addToNearby, clients, my) => {
		for (let socket of clients) {
			...
			if (boolean) {
				if (!socket.nearby.includes(my) && addToNearby)
					(my.onRender = true), socket.nearby.push(my);
...
global.becomeBulletChildren = (socket, player, exit, newgui) => {
	...
	if (a !== undefined && a !== null) {
		...
		newchildren = newchildren.filter(
			e => e.id !== a.id && e !== null && e.master === a.master
		);
...
global.loadAllMockups = (logText = true) => {
	...
	if (logText)
		console.log('Finished created ' + mockupData.length + ' MockupEntities.');
	if (logText)
		console.log(
			'Mockups generated in ' +
				util.rounder(mockupsLoadEndTime - mockupsLoadStartTime, 3) +
				' milliseconds.\n'
		);
};
